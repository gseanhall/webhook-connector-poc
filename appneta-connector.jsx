...
const APPNETA_FIELDS = [
  { name: "id", desc: "ID of the webhook or connector" },
  { name: "type", desc: "Cleared (0), Raised (1)" },
  { name: "alarmId", desc: "Alarm ID" },
  { name: "alarmRuleId", desc: "Alarm rule ID" },
  { name: "alarmRuleName", desc: "Alarm rule name" },
  { name: "alarmSeverity", desc: "Alarm severity" },
  { name: "alarmMessage", desc: "Alarm message" },
  { name: "alarmLink", desc: "Alarm details deeplink" },
  { name: "itemId", desc: "Item ID" },
  { name: "itemName", desc: "Item name: Path name" },
  { name: "itemType", desc: "Item type: web/network" },
  ...
  { name: "monitoringPolicyId", desc: "Monitoring policy id" },
  { name: "monitoringPolicyName", desc: "Monitoring policy name" },
  { name: "monitoringPolicyGroupId", desc: "Monitoring policy group id" },
  { name: "monitoringPolicyGroupName", desc: "Monitoring policy group name" },
  ...
  { name: "monitoringPointType", desc: "Monitoring Point type" },
  { name: "alarmCondition", desc: "Alarm condition" },
];
...
const TEMPLATES = {
  ...
  "providerLink": "{{alarmLink}}",
  "raisedTime": "{{raisedTime}}",
  "rule": "{{alarmRuleName}}",
  "ruleId": "{{alarmRuleId}}",
  "state": "{{mapValue type '1' 'RAISED' '0' 'CLEARED'}}",
  "target": "{{target}}",
  "monitoringPolicyId": "{{monitoringPolicyId}}",
  "monitoringPolicyName": "{{monitoringPolicyName}}",
  "monitoringPolicyGroupId": "{{monitoringPolicyGroupId}}",
  "monitoringPolicyGroupName": "{{monitoringPolicyGroupName}}",
  ...
  "blocks": [
    {
      ...
      "text": {
        "type": "plain_text",
        "text": "AppNeta Alarm: {{alarmRuleName}}"
      }
    },
    {
      "type": "section",
      "fields": [
        { "type": "mrkdwn", "text": "*Status:* {{type}}" },
  ...
  "dedup_key": "{{alarmId}}",
  "payload": {
    "summary": "{{alarmRuleName}}: {{alarmMessage}}",
    "source": "appneta",
    "severity": "{{alarmSeverity}}",
    "component": "{{itemName}}",
    "group": "{{monitoringPolicyName}}",
    "class": "{{itemType}}",
    "custom_details": {
      "alarm_id": "{{alarmId}}",
      "target": "{{target}}",
  ...
  "themeColor": "FF0000",
  "summary": "AppNeta Alarm: {{alarmRuleName}}",
  "sections": [
    {
      "activityTitle": "{{alarmRuleName}}",
      "activitySubtitle": "{{alarmMessage}}",
      "facts": [
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: `Map AppNeta webhook fields into this target JSON payload structure using {{fieldName}} template syntax. Follow any inline comments as instructions:\n\n${trimmed}` }],
        }),
      });
...
export default function ConnectorPage() {
  ...
  const SAMPLE_DATA = {
    id: "12345", type: "1", alarmId: "AL-9876", alarmRuleId: "AR-111",
    alarmRuleName: "High Latency Alert", alarmSeverity: "CRITICAL",
    alarmMessage: "Network latency exceeded 200ms threshold on path US-East → EU-West",
    alarmLink: "https://app.appneta.com/alarm/AL-9876",
    itemId: "PATH-555", itemName: "US-East → EU-West", itemType: "network",
    raisedTime: "2026-04-13T14:30:00Z", clearedTime: "",
    firstViolationTime: "2026-04-13T14:28:00Z",
    monitoringPolicyId: "MP-200", monitoringPolicyName: "Production Network Policy",
    monitoringPolicyGroupId: "MPG-50", monitoringPolicyGroupName: "Global Policies",
    target: "eu-west-gateway.example.com",
    monitoringPointId: "MPOINT-10", monitoringPointName: "US-East-1",
    monitoringPointType: "Enterprise", alarmCondition: "latency > 200ms",
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (result?.value) setTestTargets(JSON.parse(result.value));
  ...
  const saveTargets = async (targets) => {
    ...
    try { await window.storage.set(STORAGE_KEY, JSON.stringify(targets)); } catch (e) { console.error("Save failed:", e); }
  };
  ...
  const handleHandlebarsRender = () => {
    ...
    try {
      const template = window.Handlebars.compile(processed);
      const rendered = template(SAMPLE_DATA);
